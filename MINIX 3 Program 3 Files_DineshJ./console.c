...
PUBLIC unsigned shift_over  = 0;
PUBLIC u16_t previous[CONS_RAM_WORDS]; /* buffer holds copy of line */
PUBLIC unsigned tracker     = 0; /* tracks position in "previous" */
...
/*		if(tracker >= 80)  tracker = 0;

                if(shift_over) {
		  i    = 0;
		  copy = 0;
		  while(shift_over + i > 0) {
		    cons->c_ramqueue[copy + 1] = 
		      previous[(tracker-shift_over) + copy];
		    cons->c_rwords++;
                    i--;
		    copy++;
		  }
		  shift_over--;
                  cons->c_ramqueue[0] = cons->c_attr | (c & BYTE);
		  cons->c_rwords++;
		}
		else {
		  previous[tracker++] = cons->c_attr | (c & BYTE); */
