 ...
 *				hole_dmp				     *
 *===========================================================================*/
PUBLIC void hole_dmp(void)
{
	static struct pm_mem_info pmi;
	int h, n = 0;
	static int prev_h = 0;
	static int largest_bytes, total_bytes;
	...
	printf("-where does the hole begin-   -how big is the hole-\n"); 
	for(h = prev_h; h < NR_HOLES; h++) {
		if(pmi.pmi_holes[h].h_base && pmi.pmi_holes[h].h_len) {
			int bytes;
			bytes = (pmi.pmi_holes[h].h_len << CLICK_SHIFT);
			printf("%0x: %6d kB \n",
				pmi.pmi_holes[h].h_base << CLICK_SHIFT, 
				bytes / 1024);
