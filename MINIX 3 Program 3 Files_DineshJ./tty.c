...
PRIVATE struct termios termios_defaults = {
  TINPUT_DEF, TOUTPUT_DEF, TCTRL_DEF, TLOCAL_DEF, TSPEED_DEF, TSPEED_DEF,
  {
	...
	TREPRINT_DEF, TLNEXT_DEF, TDISCARD_DEF, TMOVE, TRESET,
  },
};
 ...
 */
  ...
  for (ct = 0; ct < count; ct++) {
	...
	if (!(tp->tty_termios.c_lflag & ICANON)) {
        ...
        switch (s) {
          case INIT:
            if (tp->tty_termios.c_cc[VMOVE] == ch) { 
	      s = CTRL_Y;
            }
	    else {
	  ...
	  case CTRL_Y:
	    ...
	    break;
	  case NUM:
	    if (tp->tty_termios.c_cc[VRESET] == ch) {
	      s = INIT;
	    }
	    else {
              tp->tty_incount++;
	      ...
	      *(tp->tty_inhead - num) = ch;
	      tp->tty_inhead++; 
	      for(j = 0; j < tp->tty_incount - 1; j++) {
		rawecho(tp, '\b');
		rawecho(tp, ' ');
		rawecho(tp, '\b');
	      }
	      for(k = 0; k < tp->tty_incount; k++) {
		(void) tty_echo(tp, *(tp->tty_inhead - tp->tty_incount + k));
	      }
	    }
	    break;
	}
