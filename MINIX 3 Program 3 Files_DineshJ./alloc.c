...
PRIVATE struct hole *previous_hole; /* last hole prev pointer */
...
PUBLIC  phys_clicks swap_base;	/* memory offset chosen as swap base */
 ...
 */
  ...
  register struct hole *max_hole = NIL_HOLE;

  do {
        prev_ptr      = NIL_HOLE;
	hp            = hole_head;
	max_hole      = hole_head;
	previous_hole = NIL_HOLE;

          /* swap_base = memory offset chosen as swap base */
	while (hp != NIL_HOLE && hp->h_base < swap_base) {
		  /* how big is the hole? */
		  /* vs. amount of memory requested. */
		if (hp->h_len >= clicks) {
                	if (hp->h_len > max_hole->h_len) {
				max_hole      = hp;
				previous_hole = prev_ptr;
			}
		}
		prev_ptr = hp;
		hp = hp->h_next;
	...
	if (max_hole != NIL_HOLE && max_hole->h_base < swap_base) {
		hp       = max_hole;    /* re-assign max to hp */ 
		prev_ptr = previous_hole;
