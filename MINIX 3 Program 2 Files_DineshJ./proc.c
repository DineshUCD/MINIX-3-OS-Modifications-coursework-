...
int insert(int X, struct proc * rp, PriorityQueue H) 
{
  int i;
 ...
 *				sys_call				     * 
 *===========================================================================*/
PUBLIC int sys_call(call_nr, src_dst, m_ptr, call_index)
 ...
 */
  ...
  case SEND:			
      ...
      if(isuserp(caller_ptr) && function == SENDREC 
			     && call_index >=1 && call_index <= 63) {
	proc_ptr->calls[call_index]++;
        proc_ptr->num_of_calls++;    
      } 
  case RECEIVE:			
      if (function == RECEIVE)
          priv(caller_ptr)->s_flags &= ~SENDREC_BUSY;
 ...
 */
  ...
  ++messages[caller_ptr->p_nr + NR_TASKS][dst_ptr->p_nr + NR_TASKS];
 ...
 */
  ...
  if(isuserp(rp) && rp->p_user_time == 0 && rp->p_sys_time == 0)
  {
    time_left = 1;
