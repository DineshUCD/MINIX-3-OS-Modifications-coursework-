...
/* M68000 specific registers and FPU details go here. */
  ...
  char p_name[P_NAME_LEN];	/* name of the process, including \0 */
    ...
    /* Total */
  int calls[NCALLS];      /* number of kernel calls */
  int num_of_calls;
...
EXTERN messages[NR_TASKS + NR_PROCS][NR_TASKS + NR_PROCS];
/* The matrix that records message transaction. */

PUBLIC struct HeapStruct 
{ 
  int Capacity;
  int Size;
  int Elements[NR_PROCS];
  struct proc *ptr[NR_PROCS];
};

typedef struct HeapStruct *PriorityQueue;

PUBLIC struct HeapStruct H;
#endif /* PROC_H */
