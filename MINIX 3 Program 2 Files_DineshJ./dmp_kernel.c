...
PUBLIC int messages[NR_TASKS + NR_PROCS][NR_TASKS + NR_PROCS];
/*===========================================================================*
 *				timing_dmp				     *
 *===========================================================================*/
...
PUBLIC void magic_dmp()
{
  ...
  if(sys_getmagic(messages) != OK)
    return;

  printf("Messages process X sends to process Y\n");
  printf("Format X: Y->#...\n");
  ...
  for(rp = oldrp; rp < END_PROC_ADDR ; rp++)
  {
    if(isemptyp(rp))
      continue;
    if(++row_limit > 10) break;
    printf("%s [%d]: ", rp->p_name, rp->p_nr);
    col_limit = 0;
    for(sent_to = 0; sent_to < NR_TASKS + NR_PROCS; sent_to++)
    {
      if(isemptyp(&proc[sent_to]))
	continue;
      if(++col_limit > 10) break;
      printf("%d->%d ", sent_to, messages[rp->p_nr + NR_TASKS][sent_to]);
    }
    printf("\n");
  }
 ...
 *				calls_dmp				     *
 *===========================================================================*/
PUBLIC void calls_dmp()
{
  register struct proc *rp;
  static struct proc *oldrp = BEG_USER_ADDR;
  int r, col_limit = 0, row_limit = 0, call_num = 0;
  ...
  printf("System calls invoked by each user process.\n");
  ...
  for(rp = oldrp; rp < END_PROC_ADDR; rp++)
  {
    ...
    printf("%s[%d] Total %d: ", rp->p_name, rp->p_nr, rp->num_of_calls);
    
    col_limit = 0;
    for(call_num = 0; call_num < NCALLS; call_num++)
    {
      if(rp->calls[call_num] > 0)
      {
        printf("#%d: %d ", call_num, rp->calls[call_num]);
	col_limit++;
      }
      if(col_limit > 10) break;
    } 
  ...
  if(rp == END_PROC_ADDR) rp = BEG_USER_ADDR; else printf("--more--\r");
  oldrp = rp;
}
